...
const TVWatch = () => {
  const { id } = useParams();
  const { data, error } = useQuery(["watchTV", id], () => getWatchTV(Number(id)));
  ...
  const currentEpisode = currentSeason?.episodes.find((episode) => episode.episode_number === episodeId);
  ...
  return (
    <FilmWatch {...data} media_type="tv" seasonId={seasonId} episodeId={episodeId} currentEpisode={currentEpisode} />
  );
};
