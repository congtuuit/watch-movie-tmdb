...
const ExploreResult = ({ currentTab, config }) => {
  ...
  } = useInfiniteQuery(["explore-result-movie", config], ({ pageParam = 1 }) => getExploreMovie(pageParam, config), {
  ...
  const {
    ...
    ({ pageParam = 1, queryKey }) => getExploreTV(pageParam, queryKey[1]),
    {
      getNextPageParam: (lastPage) => (lastPage.page + 1 <= lastPage.total_pages ? lastPage.page + 1 : undefined),
    }
  );
  ...
  return (
    ...
    >
      {currentTab === "movie" && (
        <ExploreResultContent data={movies?.pages} fetchNext={fetchNextPageMovie} hasMore={hasNextPageMovie} />
      )}

      {currentTab === "tv" && (
        <ExploreResultContent data={tvs?.pages} fetchNext={fetchNextPageTv} hasMore={hasNextPageTv} />
      )}
    </div>
  );
};
