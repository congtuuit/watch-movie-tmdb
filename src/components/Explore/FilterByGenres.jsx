...
const FilterByGenres = ({ currentTab }) => {
  const { isLoading, data, isError, error } = useQuery(["genres"], getRecommendGenres2);
  ...
  return (
    ...
    >
      {data[currentTab === "movie" ? "movieGenres" : "tvGenres"].map((genre) => (
        <li key={genre.id}>
          <button
            ...
            className={`px-4 py-1 border border-[#989898] rounded-full hover:brightness-75 transition duration-300 inline-block ${
              searchParam.getAll("genre").includes(String(genre.id)) && "bg-primary text-white"
            }`}
          >
            {genre.name}
          </button>
