...
const FilmListViewForBookmarkAndHistory = ({ films, isLoading, pageType }) => {
  ...
  const selectAllHandler = () => {
    ...
    if (currentTab === "all") {
      ...
      setSelections(films.filter((film) => film.media_type === "tv").map((film) => film.id));
    } else if (currentTab === "movie") {
      setSelections(films.filter((film) => film.media_type === "movie").map((film) => film.id));
    }
  };
  ...
  const clearSelection = () => {
    ...
    const editedFilms = films.filter((film) => selections.indexOf(film.id) === -1);
        ...
        {isShowPrompt && (
          <>
            <div className="fixed top-[30%] md:left-[40%] left-[5%] right-[5%] md:w-[400px] z-40 bg-dark-lighten rounded-md min-h-[100px] shadow-md px-3 py-5">
              ...
              <p className="text-center mb-[2px]">This will remove your films from this {pageType} list.</p>
            ...
            <BlackBackdrop onCloseBlackBackdrop={() => setIsShowPrompt(false)} className="!z-30" />
          </>
        )}
      </div>
