...
const FilmWatch = ({ detail, recommendations, detailSeasons, media_type, seasonId, episodeId, currentEpisode }) => {
      ...
      if (!isAlreadyStored) {
        updateDoc(doc(db, "users", currentUser.uid), {
          recentlyWatch: arrayUnion({
            ...
            ...(media_type === "tv" && { name: detail?.name }),
          }),
        });
      } else {
        const updatedRecentlyWatch = docSnap
          .data()
          ?.recentlyWatch.filter((film) => film.id !== detail?.id)
          .concat({
            poster_path: detail?.poster_path,
            id: detail?.id,
            vote_average: detail?.vote_average,
  ...
  const getTitle = (detail) => {
    if (detail) {
      return `Watch: ${detail.title || detail.name} ${
  ...
  const poster = (detail) => {
    if (detail) {
      const url = resizeImage(detail.poster_path, "w342");
      return url;
    }
    return "";
  };
  ...
  return (
    <>
      <MetaInfo title={getTitle(detail)} description={detail?.tagline} image={poster(detail)} />
          ...
          <div className="relative h-0 pb-[56.25%]">
            ...
            {detail && (
              <iframe
                ...
                src={media_type === "movie" ? embedMovie(detail.id) : embedTV(detail.id, seasonId, episodeId)}
          ...
          <div className="mt-5 pb-8">
            <div className="flex justify-between md:text-base text-sm">
                ...
                {detail && (
                  <div className="flex gap-5 mt-5">
                    <div className="flex gap-2 items-center">
                      ...
                      {media_type === "movie" && <p>{detail.vote_average.toFixed(1)}</p>}
                      {media_type === "tv" && <p>{currentEpisode?.vote_average.toFixed(1)}</p>}
                    </div>
                    <div className="flex gap-2 items-center">
                      <AiTwotoneCalendar size={25} className="text-primary" />
                      <p>
                        {media_type === "movie" && new Date(detail.release_date).getFullYear()}
                        {media_type === "tv" && new Date(currentEpisode.air_date).getFullYear()}
                      </p>
                    </div>
                  </div>
                )}
              ...
              {media_type === "tv" && currentEpisode && (
                <div className="flex-1">
                  <h2 className="md:text-xl italic uppercase text-gray-200 mt-2 text-right">{currentEpisode.name}</h2>
            ...
            {detail && (
              <ReadMore limitTextLength={300} className="md:text-lg text-base mt-1">
                {media_type === "movie" ? detail.overview : currentEpisode?.overview}
              </ReadMore>
            )}
          </div>
        ...
        <div className="shrink-0 md:max-w-[400px] w-full relative px-6">
          ...
          {media_type === "tv" && (
            <div className="md:mt-24">
              ...
              <SeasonSelection detailSeasons={detailSeasons} seasonId={seasonId} episodeId={episodeId} />
            </div>
          )}
        </div>
      </div>
