  ...
  return (
    <>
      <div className="relative z-[39]">
        <button
          onClick={() =>
            showOptionFor === singleDoc.id ? setShowOptionFor(undefined) : setShowOptionFor(singleDoc.id)
          }
        ...
        {showOptionFor === singleDoc.id && (
          <div className="absolute -left-8 w-[70px] flex flex-col gap-1 bg-dark-lighten-2 rounded-md px-3 py-2 shadow-md">
            ...
            {(!currentUser || currentUser.uid !== singleDoc.data()?.user.uid) && (
              <button
                onClick={() => setCommentHiden((prev) => prev.concat(singleDoc.id))}
      ...
      {showOptionFor === singleDoc.id && (
        ...
        <div onClick={() => setShowOptionFor(undefined)} className="fixed top-0 left-0 w-full h-full z-[35]"></div>
      )}
      <div
        // @ts-ignore
        ref={show}
        ...
        {isShowPrompt && (
          <>
            <div className="fixed top-[30%] md:left-[40%] md:right-auto left-[5%] right-[5%] md:w-[400px] z-50 bg-dark-lighten rounded-md min-h-[100px] shadow-md px-3 py-5">
              ...
              <p className="text-white text-xl text-center font-medium mb-4">You are about to remove this comment</p>
              <p className="text-center mb-[2px]">This will remove your films and cannot recover</p>
            ...
            <BlackBackdrop onCloseBlackBackdrop={() => setIsShowPrompt(false)} className="!z-40" />
          </>
        )}
      </div>
    </>
