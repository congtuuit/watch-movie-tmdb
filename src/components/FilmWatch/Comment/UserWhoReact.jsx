...
const UserWhoReact = ({ docData, isShowReactionData, setIsShowReactionData }) => {
  const [infoReactionUser, setInfoReactionUser] = useState([]);

  useEffect(() => {
    const infoReactionUser = Object.entries(docData.reactions).map(async (entry) => {
  ...
  return (
    <>
      {isShowReactionData && (
        <BlackBackdrop onCloseBlackBackdrop={() => setIsShowReactionData(false)} className="!z-40">
          <div
          ...
          >
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setIsShowReactionData(false)}>
                <AiOutlineClose size={20} className="text-white hover:brightness-75 transition duration-300" />
              </button>
            </div>
            ...
            <ul className="flex flex-col gap-3">
              {infoReactionUser.map((item, index) => (
                <li key={index} className="flex justify-between items-center">
                  ...
                  <div>
                    {item.reaction === "like" && <AiTwotoneLike className="text-blue-500" size={20} />}
                    {item.reaction === "love" && <AiFillHeart className="text-red-500" size={20} />}
                    {item.reaction === "haha" && <BsEmojiLaughingFill className="text-yellow-500" size={20} />}
                    {item.reaction === "wow" && <FaSurprise className="text-green-500" size={20} />}
                    {item.reaction === "sad" && <FaSadTear className="text-purple-500" size={20} />}
                    {item.reaction === "angry" && <FaAngry className="text-orange-500" size={20} />}
                  </div>
                </li>
              ))}
            </ul>
