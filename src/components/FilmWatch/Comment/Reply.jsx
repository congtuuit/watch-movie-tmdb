...
const Reply = ({ commendId }) => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useCollectionQuery(commendId, query(collection(db, `replyTo-${commendId}`), orderBy("createdAt", "desc")));

  // Auto comment
  useEffect(() => {
    getDocs(collection(db, "replyTo-admin")).then((docSnapshot) => {
      if (!docSnapshot.docs.some((doc) => doc.data()?.user.uid === "Z3eRARZ9jlftBLA6u0g8MWABkwo2")) {
        addDoc(collection(db, "replyTo-admin"), {
          ...
          createdAt: Timestamp.fromDate(new Date("Sat Aug 06 2022 10:10:32 GMT+0700 (Indochina Time)")),
          isEdited: false,
        });
      }
    });
