...
const ReactionInfo = ({ docData }) => {
  const [isShowReactionData, setIsShowReactionData] = useState(false);

  const convertReaction = (reactions) => {
    const countNumberOfEachReaction = Object.values(reactions).reduce((acc, current) => {
      acc[current] = 1 + (acc[current] || 0);

      return acc;
    }, {});

    const sortedByNumberOfReaction = Object.entries(countNumberOfEachReaction).sort((a, b) => b[1] - a[1]);
        ...
        <>
          ...
          >
            {convertReaction(docData.reactions)
              .slice(0, 3)
              .map((reactionEntry, index) => (
                <div key={index}>
                  {reactionEntry[0] === "like" && (
                    <AiTwotoneLike
                      className={`relative text-blue-500 -ml-2 ${index === 0 ? "z-30" : index === 1 ? "z-20" : "z-10"}`}
                      size={20}
                    />
                  )}
                  {reactionEntry[0] === "love" && (
                    <AiFillHeart
                      className={`relative text-red-500 -ml-2 ${index === 0 ? "z-30" : index === 1 ? "z-20" : "z-10"}`}
                      size={20}
                    />
                  )}
          ...
          <ul className="flex flex-col gap-2 peer-hover:opacity-100 peer-hover:visible opacity-0 invisible transition duration-300 absolute -right-24 top-[90px] bg-dark-lighten-2/60 px-3 py-2 rounded-md shadow-md z-10">
            {convertReaction(docData.reactions).map((reactionEntry, index) => (
              <li key={index}>
                {reactionEntry[0] === "like" && (
                  <div className="flex gap-2 items-center">
                    <AiTwotoneLike className={`relative text-blue-500  `} size={20} />
                    <p>{reactionEntry[1]}</p>
                  </div>
                )}
                {reactionEntry[0] === "love" && (
                  <div className="flex gap-2 items-center">
                    <AiFillHeart className={`relative text-red-500  `} size={20} />
                    <p>{reactionEntry[1]}</p>
                  </div>
                )}
                {reactionEntry[0] === "haha" && (
                  <div className="flex gap-2 items-center">
                    <BsEmojiLaughingFill className={`relative text-yellow-500 `} size={20} />
                    <p>{reactionEntry[1]}</p>
                  </div>
                )}
                {reactionEntry[0] === "wow" && (
                  <div className="flex gap-2 items-center">
                    <FaSurprise className={`relative text-green-500 `} size={20} />
                    <p>{reactionEntry[1]}</p>
                  </div>
                )}
                {reactionEntry[0] === "sad" && (
                  <div className="flex gap-2 items-center">
                    <FaSadTear className={`relative text-purple-500  `} size={20} />
                    <p>{reactionEntry[1]}</p>
                  </div>
                )}
                {reactionEntry[0] === "angry" && (
                  <div className="flex gap-2 items-center">
                    <FaAngry className={`relative text-orange-500  `} size={20} />
                    <p>{reactionEntry[1]}</p>
                  </div>
                )}
              </li>
