...
const Season = ({ season, seasonId, episodeId }) => {
  const [isSeasonExpanded, setIsSeasonExpanded] = useState(season.season_number === 1);
    ...
    >
      ...
      >
        ...
        <div className="flex-grow text-left">
          ...
          <p className={` transition duration-300 ${season.season_number === seasonId && "text-white"}`}>
                ...
                <div className="shrink-0 max-w-[15px] w-full">
                  <p
                    className={`text-white font-medium transition duration-300 ${
                      episode.episode_number === episodeId && season.season_number === seasonId && "!text-primary"
                    }`}
                  >
                ...
                <div className="flex-grow">
                  <p
                    className={`transition duration-300 text-sm ${
                      episode.episode_number === episodeId && season.season_number === seasonId && "text-white"
                    }`}
                  >
                    {episode.name}
                  </p>
...
const SeasonSelection = ({ detailSeasons, seasonId, episodeId }) => {
    ...
    >
      {detailSeasons &&
        detailSeasons.map((season) => (
          <Season key={season.id} season={season} seasonId={seasonId} episodeId={episodeId} />
        ))}
      {!detailSeasons && (
        <div>
