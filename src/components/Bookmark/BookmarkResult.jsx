...
const BookmarkResult = ({ films, isEditing, selections, setSelections, isLoading, pageType }) => {
  return (
    <>
              ...
              <button
                onClick={() =>
                  setSelections((prev: number[]) =>
                    prev.includes(item.id) ? prev.filter((id: number) => id !== item.id) : prev.concat(item.id)
                  )
                }
