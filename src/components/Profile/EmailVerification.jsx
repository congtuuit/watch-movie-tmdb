...
const EmailVerification = ({ setIsUpdating }) => {
    ...
    <>
      ...
      {isVerificationEmailSent && (
        <>
          ...
          <BlackBackdrop onCloseBlackBackdrop={() => setIsVerificationEmailSent(false)} />
        </>
      )}

      <div className="mt-10 flex justify-between max-w-[600px]">
        <p className="text-white text-lg">
          {!currentUser?.emailVerified ? "Your email is not verified yet." : "Your email is verified."}
        </p>
        ...
        {!isShowSentButton && !currentUser?.emailVerified && <p className="text-lg">Waiting for verify</p>}
      </div>
    </>
  );
};
