...
const FilmDetail = (props) => {
  const { similar, videos, detail, ...others } = props;
  ...
  useEffect(() => {
    ...
    const unsubDoc = onSnapshot(doc(db, "users", currentUser.uid), (doc) => {
      setIsBookmarked(doc.data()?.bookmarks.some((item) => item.id === detail?.id));
    });
  ...
  const bookmarkedHandler = async () => {
    ...
    toast.success(`${!isBookmarked ? "This film is now bookmarked" : "This film is removed from your bookmarks"}`, {
      position: "top-right",
      autoClose: 2000,
      hideProgressBar: false,
      closeOnClick: true,
  ...
  return (
    <>
      {detail && (
        <MetaInfo
          title={`${detail.title || detail.name} | Moonlight`}
      ...
      <div className="flex flex-col md:flex-row">
        ...
        <div className="flex-grow min-h-screen">
          ...
          {!detail && <Skeleton className="h-[400px] rounded-bl-2xl "></Skeleton>}
          {detail && (
            <div
              style={{
                ...
                <div className="flex flex-col md:flex-row bottom-[-85%] md:bottom-[-20%]  items-start tw-absolute-center-horizontal w-full max-w-[1000px]">
                  ...
                  <div className="flex-grow md:ml-14 ml-6 mt-6 md:mt-0">
                    <div className="md:h-28 flex items-end">
                      <h1 className=" text-white text-[45px] font-bold leading-tight">{detail.title || detail.name}</h1>
                    </div>
            ...
            {!isMobile && (
              <div className="shrink-0 md:max-w-[150px] w-full flex items-center md:flex-col justify-center flex-row gap-20 mt-20 md:border-r border-dark-lighten pt-16">
                <div className="flex flex-col gap-6 items-center">
                  ...
                  {isMobile && detail && <p className="text-2xl -mt-3">{detail.vote_average.toFixed(1)}</p>}
                </div>
                  ...
                  {detail && (
                    <>
                      ...
                      <div className="flex gap-2 items-center">
                        {detail.media_type === "movie" && <p className="text-2xl">{detail.runtime}</p>}
                        {detail.media_type === "tv" && <p className="text-2xl">{detail.episode_run_time[0]}</p>}
                        <span>min</span>
                      </div>
                    </>
                  )}
                ...
                {videos &&
                  videos.slice(0, 2).map((video) => (
                    <li key={video.id}>
                      ...
                      <p className="mt-3 text-lg whitespace-nowrap overflow-hidden text-ellipsis">{video.name}</p>
                    </li>
                  ))}
                {!videos &&
