...
import { SortReview } from "./SortReview.js";

const ReviewContent = ({ reviews, type }) => {
        ...
        <li key={review.id} className="flex gap-7">
          ...
          <div className="flex-grow">
            <div className="flex justify-between">
              ...
              <StarRating star={Math.round(review.author_details.rating / 2)} maxStar={5} />
            </div>
            <ReadMore limitTextLength={150}>{review.content}</ReadMore>
            <p className="text-right text-base">{calculateTimePassed(new Date(review.created_at).getTime())}</p>
          </div>
        </li>
      ))}
    </ul>
...
const ReviewTab: FC<ReviewTabProps> = ({ reviews }) => {
  ...
  return (
    <>
      ...
      <div>
        {reviews.length === 0 && <p className="text-center text-white text-lg">There is no reviews yet.</p>}
        {reviews.length > 0 && <ReviewContent reviews={reviews} type={reviewSortType} />}
      </div>
    </>
  );
};
