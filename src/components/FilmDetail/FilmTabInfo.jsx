...
const FilmTabInfo = ({ detail, credits, reviews }) => {
  ...
  return (
    <>
      <ul className="flex gap-10 text-gray-400 text-lg justify-center">
        {tabButtons.map((btnName, index) => (
          <li key={index} className={detail?.media_type === "movie" ? "last:hidden" : ""}>
            <button
              onClick={() => setCurrentTab(btnName)}
              className={`hover:text-white transition duration-300 pb-1 ${
                currentTab === btnName && "font-medium -translate-y-2 border-b-2 border-primary text-white"
              }`}
            >
      ...
      <div className="mt-10 text-lg">
        {currentTab === "overall" && (
          <>
            {detail && <p className="text-xl italic mb-8 text-white text-center">{detail.tagline}</p>}
            ...
            {detail && (
              <>
                ...
                {detail.media_type === "movie" && <p>Release date: {detail.release_date}</p>}
                {detail.media_type === "tv" && <p>Last air date: {detail.last_air_date}</p>}
                <p>
                  Spoken language:
                  {detail.spoken_languages.map((language, index) => `${index ? ", " : ""} ${language.english_name}`)}
                </p>
              </>
            )}
          </>
